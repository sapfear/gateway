...
function ChargePointSimulator(uri, identifier, protocol, transport,
  ...
  Plugins.setAPIFields(transport, 'cp', this.protocol, this.chargePointId);
...
ChargePointSimulator.prototype = {
	...
	clientConnectionState: function(){
...
function commandCPtoCS(cpId, procName, args) {
  ...
  if(cpId == -1) {
    ...
    if(cpId == -1) {
      console.log("CPtoCS Error: cannot use shortened command : there is no or more"+
